...
@app.route('/archive')
def archive_panel():
    """Renders the Archive panel page."""
    if 'user_id' not in session:
        return redirect('/login')
    # Renders the new static panels page with the specific panel active
    return render_template('panels.html', panel_name='Archive')

@app.route('/admin')
def admin_panel():
    """Renders the Admin panel page (Static placeholder)."""
    if 'user_id' not in session:
        return redirect('/login')
    # Renders the new static panels page with the specific panel active
    return render_template('panels.html', panel_name='Admin')

@app.route('/api/archive/history', methods=['GET'])
def get_archive_history():
    """Fetches saved summaries for the user from the database."""
    ...
    try:
        with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
            # Query the usersummaries table for saved items
            cursor.execute("""
                SELECT 
                    us.summary_text, 
                    us.saved_at, 
                    s.query AS initial_search_query
            ...
            # Convert fetched results to a list of dicts for JSON
            archive_list = [{
                'summary': record['summary_text'],
                'saved_at': record['saved_at'].isoformat() if record['saved_at'] else 'N/A',
                'query': record['initial_search_query']
        ...
        print("❌ Get archive history error:", e)
        import traceback
        traceback.print_exc()
        return jsonify({'status': 'error', 'message': 'Failed to retrieve archive history'}), 500
    finally:
        if db:
            db.close()
#d----------------------------------------------------------------------------------
